<div class="mx-0 sm:mx-0 border border-gray-100 rounded-md">
    ...
    {{-- @if ($search != '')
    <span class="text-gray-500 mb-2 text-sm italic font-medium">
        Nombre de publication trouvée : <span class="font-semibold">({{ $posts->total() }})</span>
    </span>
    @endif --}}
    ...
    <center>
        <div class="w-3/4">
            <div class="relative rounded-3xl shadow self-center">
                ...
                <input type="text" wire:model.debounce.500ms="search"
                    class="focus:ring-blue-600 focus:border focus:border-blue-600 focus:shadow-2xl block w-full pl-10 pr-12 sm:text-sm focus:z-20 font-medium border-gray-400 py-2 rounded-3xl"
        ...
        <div class="mt-2 text-gray-500 font-serif">
            @if ($search != "")
            @if ($posts->total() != 0)
            Nombre de publication trouvée : <span class="font-bold"> {{ $posts->total() }} </span>
            @endif
            @endif
        </div>
    </center>
        ...
        <div class="max-w-7xl px-4 sm:px-6 lg:px-12 mx-auto space-y-7">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-2 lg:gap-8">
                                ...
                                <div class="flex -space-x-6">
                                    ...
                                    <div class="relative" style="z-index: 3;">
                                        ...
                                        <img x-on:mouseover="tooltip = 1" x-on:mouseleave="tooltip = 0"
                                            ...
                                            src="{{ is_null($post->user->getFirstMedia('avatars')) ? '/images/no-image-user.jpg' : $post->user->getFirstMedia('avatars')->getUrl() }}"
                                            alt="profil">
                                    </div>
                                ...
                                <div class="px-2 bg-gray-200 rounded-2xl border border-gray-200">
                                    <a href="" @click.prevent="toggleComment"
                                        ...
                                        <x-heroicon-o-chat-alt class="h-6 w-6" /> <span
                                            class="text-blue-700 rounded text-xs px-1 py-1">{{ $post->getCommentsNumber() }}
                                        </span>
                                    </a>
                                </div>
                ...
                <div class="mb-20 py-8 text-sm font-medium flex flex-col items-center justify-center h-72">
                    <x-heroicon-o-emoji-sad class=" h-14 w-14 text-gray-500" />
                    <p class="text-gray-500">
                        Désolé ! Aucune publication trouvée !
                    </p>
                </div>
